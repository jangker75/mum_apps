<?php namespace App\Http\Controllers;
	...
	class AdminKotaController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
			...
			$this->index_button[] = ['label'=>'Import Data','url'=>action('AdminKotaController@getImportXls'),'icon'=>'fa fa-upload'];
	    	...
	    	\Excel::filter('chunk')->selectSheetsByIndex(0)->load(public_path('import/'.$file->getClientOriginalName()))->chunk(500,function($result) { 
			    ...
			    foreach($result->toArray() as $row) {
						...
						$prov = DB::table('provinsi')
								->where('kode',$row['kode_provinsi'])->orderBy('id','asc')
								->first();
						$a['provinsi_id'] = $prov->id;
				    	$a['kode'] = $row['kode_kota'];
				    	$a['nama'] = $row['nama'];
						$a['keterangan'] = $row['keterangan'];
						 ...
						 if ($prov) {
							...
							->first();
							if (!$kota) {
								DB::table('kota')->insert($a);
								$sukses++;
							} else{
								echo $no.' | '.$row['kode_kota'].' | '.$row['nama'].' | Failed import, kode kota sudah ada<br/>';
								$no++;$error++;
								}
						}
							else{
								echo $no.' | '.$row['kode_provinsi'].' | '.$row['nama'].' | Failed import, kode provinsi tidak ada<br/>';
								$no++;$error++;
							}
