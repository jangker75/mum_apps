<?php namespace App\Http\Controllers;
	...
	class AdminProvinsiController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
			...
			$this->index_button[] = ['label'=>'Import Data','url'=>action('AdminProvinsiController@getImportXls'),'icon'=>'fa fa-upload'];
		...
		public function postImportXls() {
	    	...
	    	\Excel::filter('chunk')->selectSheetsByIndex(0)->load(public_path('import/'.$file->getClientOriginalName()))->chunk(500,function($result) { 
			    ...
			    foreach($result->toArray() as $row) {
				    	...
				    	$cek = DB::table('provinsi')->where('kode',$row['kode'])->orderBy('id','asc')->first();
						if($cek){
							echo $no.' | '.$row['kode'].' | '.$row['nama'].' | Failed import, kode sudah ada<br/>';
							$no++;$error++;
						}else{
                            DB::table('provinsi')->insert($a);
                            $sukses++;
						}
