					...
					<?php
						if($s->tipe!='Text') {
							...
							foreach($soalJawaban as $sj) {
								$jumlahJawaban = DB::table('kuisoner_detail')
								->leftjoin('kuisoner','kuisoner.id','=','id_kuisoner')
								->leftjoin('member','member.id','=','kuisoner.id_member')
								->where('id_event',$event->id)
								->where('id_soal',$s->id)
								->where('id_soal_jawaban',$sj->id);
								if(g('id_kecamatan')) $jumlahJawaban->where('member.id_kecamatan',g('id_kecamatan'));
								if(g('id_kabupaten')) $jumlahJawaban->where('member.id_kabupaten',g('id_kabupaten'));							
								$jumlahJawaban = $jumlahJawaban->count();
								$dataSet[] = ['value'=>$jumlahJawaban,'jawaban'=>$sj->jawaban,'id_soal_jawaban'=>$sj->id,'keterangan'=>''];
								$totalY += $jumlahJawaban;
							}
						}else{ //end if not text

							$subQuery = DB::table("kuisoner_detail as b")
							->join("kuisoner","kuisoner.id","=","b.id_kuisoner")
							->join("member","member.id","=","kuisoner.id_member")
							->where("b.id_soal",DB::raw("kuisoner_detail.id_soal"))
							->where("b.keterangan",DB::raw("kuisoner_detail.keterangan"));
							if(g('id_kabupaten')) $subQuery->where("member.id_kabupaten",g("id_kabupaten"));
							if(g('id_kecamatan')) $subQuery->where("member.id_kecamatan",g('id_kecamatan'));
							$subQuery->select(DB::raw("COUNT(b.id)"));

							$groupText = DB::table('kuisoner_detail')
							->join('soal','soal.id','=','id_soal')
							->where('soal.tipe','Text')
							->where('id_soal',$s->id)
							->where('kuisoner_detail.keterangan','!=','')
							->select('kuisoner_detail.keterangan',
								DB::raw("(".$subQuery->toSql().") as jumlah"))
							->groupBy('kuisoner_detail.keterangan')
							->orderby('jumlah','desc')
							->take(5)
							->get();
							$totalY = 0;
							...
							foreach($groupText as $g) {
								$totalY += $g->jumlah;
								$dataSet[] = ['value'=>$g->jumlah,'jawaban'=>$g->keterangan,'id_soal_jawaban'=>'','keterangan'=>$g->keterangan];
							}
						}
						...
						foreach($dataSet as $r) {
							$new = $r;							
							$new['value'] = @round($r['value'] / $totalY * 100, 2);
							$new['label'] = $new['value'].'%';
							$new['calc'] = $r['value'].' / '.$totalY.' * 100';
							$newDataSet[] = $new;							
						}																
					...
					?>
					  <div class="box box-primary">
					  	<div class="box-header">
					  		<h3><?php echo e($s->isi); ?></h3>
					  	</div>
					  	<div class="box-body">
					  		  <?php $__currentLoopData = $newDataSet; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
					  		  <h4 style="font-weight: bold"><?php echo e($d['jawaban']); ?> (<?php echo e($d['value']); ?>%)</h4>
							  <div class="progress active" style="height: 40px;cursor: pointer" onclick="location.href='<?php echo e(url("admin/dashboard/anggota")); ?>?id_soal=<?php echo e($s->id); ?>&id_event=<?php echo e($event->id); ?>&id_soal_jawaban=<?php echo e($d['id_soal_jawaban']); ?>&keterangan=<?php echo e($d['keterangan']); ?>&id_kabupaten=<?php echo e(g("id_kabupaten")); ?>&id_kecamatan=<?php echo e(g("id_kecamatan")); ?>'">
					            <div class="progress-bar progress-bar-success progress-bar-striped" title="<?php echo e($d['value']); ?>%" role="progressbar" aria-valuenow="<?php echo e($d['y']); ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo e($d['value']); ?>%">
					              <span class="sr-only"><?php echo e($d['value']); ?>%</span>
					            </div>
					          </div>				
					          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
					  	</div>
